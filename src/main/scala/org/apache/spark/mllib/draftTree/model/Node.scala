...
import org.apache.spark.mllib.draftTree.model.Node;
...
class Node (
   ...
   */
  private[draftTree] def deepCopy(): Node = {
   ...
   */
  private[draftTree] def numDescendants: Int = if (isLeaf) {
    0
  } else {
   ...
   */
  private[draftTree] def subtreeDepth: Int = if (isLeaf) {
    0
  } else {
   ...
   */
  private[draftTree] def subtreeToString(indentFactor: Int = 0): String = {

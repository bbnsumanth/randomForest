...
package org.apache.spark.mllib.draftTree.configuration

import scala.collection.JavaConverters._

import org.apache.spark.annotation.Experimental
import org.apache.spark.mllib.draftTree.impurity.{Variance, Entropy, Gini, Impurity}
...
class Strategy (
  ...
  private[draftTree] def assertValid(): Unit = {
    ...
    require(numTrees > 0, s"Strategy requires numTrees > 0, but was given numTrees = $numTrees.")
    
    require(supportedFeatureSubsetStrategies.contains(featureSubsetStrategy),
    s"RandomForest given invalid featureSubsetStrategy: $featureSubsetStrategy." +
    s" Supported values: ${supportedFeatureSubsetStrategies.mkString(", ")}.")

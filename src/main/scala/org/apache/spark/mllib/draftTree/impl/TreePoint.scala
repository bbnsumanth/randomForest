 ...
 */
private[draftTree] class TreePoint(val label: Double, val binnedFeatures: Array[Int])
  extends Serializable {
}

private[draftTree] object TreePoint {
  ...
  private def labeledPointToTreePoint(
    ...
    while (featureIndex < numFeatures) {
      ...
      arr(featureIndex) = findBin(labeledPoint.features(featureIndex), featureArity(featureIndex),
        isUnordered(featureIndex), bins(featureIndex))
      featureIndex += 1
      ...
      if (binIndex == -1) {
        throw new RuntimeException("No bin was found for continuous feature."
         /* + " This error can occur when given invalid data values (such as NaN)." +
          s" Feature index: $binIndex.  Feature value: ${labeledPoint.features(featureIndex)}"*/)
      }
      binIndex
      ...
      if (featureValue < 0 || featureValue >= featureArity) {
        throw new IllegalArgumentException(
          s"DecisionTree given invalid data:"/* +
            s" Feature $binIndex is categorical with values in" +
            s" {0,...,${featureArity - 1}," +
